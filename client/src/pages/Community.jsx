...
const Community = () => {
  ...
  const imageLikesToggle= async(id)=>{
    try {
        ...
        <div className='bg-white h-full w-full rounded-xl overflow-y-scroll'>
          {creations.map((creation,index)=>(
            ...
            sm:max-w-1/2 lg:max-w-1/3'>
              ...
              item-end justify-end group-hover:justify-between p-3
              group-hover:bg-gradient-to-b from-transparent to-black/80 text-white
              rounded-lg'>
                <p className='text-sm hidden group-hover:block'>{creation.prompt}</p>
                <div className='flex-sm hidden group-hover:block'>
                  <p>{creation.likes.length}</p>
                  <Heart onClick={()=> imageLikesToggle(creation.id)} className={`min-w-5 h-5 hover:scale-110 cursor-pointer ${creation.likes.includes(user.id)?
